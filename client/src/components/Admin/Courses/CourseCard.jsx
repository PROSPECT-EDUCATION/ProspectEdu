...
export default function CourseCard({ course }) {
  ...
  return (
    <div className="bg-white border border-[#A7E1B2] rounded-xl overflow-hidden shadow-sm">
      ...
      {/* CONTENT */}
      <div className="p-4">
        <h2 className="font-heading text-lg text-[#124734]">{course.title}</h2>
        <p className="text-sm text-[#5B7065]">{course.short}</p>
        ...
        <div className="mt-4 flex justify-between items-center">
          ...
          <span className="text-xs text-[#5B7065]">{course.category}</span>
        </div>
      </div>
