      ...
      <div className="max-w-7xl mx-auto px-6 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {channels.map((channel, index) => (
            <div
              key={index}
              className="bg-white border border-[#A7E1B2] rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 hover:scale-[1.03]"
            >
              {/* Gradient Banner */}
              <div
                className={`h-40 ${channel.gradient} flex items-center justify-center relative`}
              >
                <Play className="w-12 h-12 text-white bg-[#124734]/70 rounded-full p-2 hover:bg-[#009846] transition" />
              </div>
