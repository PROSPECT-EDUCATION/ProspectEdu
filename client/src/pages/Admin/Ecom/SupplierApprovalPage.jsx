...
import { Search, CheckCircle2, XCircle, Ban, RotateCcw, Store, User, Phone, Mail, MapPin } from "lucide-react";
...
const Badge = ({ text, kind }) => {
  ...
  return <span className={`${base} ${map[kind] || "bg-gray-50 text-gray-700 border-gray-200"}`}>{text}</span>;
};
...
export default function SupplierApprovalPage() {
  ...
  const load = async () => {
    try {
      ...
      // backend returns array directly currently
  ...
  return (
    <div className="flex h-screen bg-[#F9FAFB] overflow-hidden">
      {/* Sidebar */}
      <div className={`fixed top-0 left-0 h-full z-40 transition-all duration-300 ${isCollapsed ? "w-20" : "w-64"}`}>
        <AdminSidebar isCollapsed={isCollapsed} setIsCollapsed={setIsCollapsed} />
      </div>

      {/* Main */}
      <div className="flex flex-col flex-1 transition-all duration-300" style={{ marginLeft: sidebarWidth }}>
        <div className="fixed top-0 bg-white shadow-sm h-[64px] flex items-center z-[999]" style={{ left: sidebarWidth, right: 0 }}>
        ...
        <div className="px-6 pt-[90px] pb-10 overflow-y-auto">
          ...
          <div className="bg-white rounded-xl shadow p-5 flex flex-col gap-4">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-gray-600">Approve, reject, or block suppliers. See all details in one place.</p>
              </div>
              ...
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-semibold border transition
                    ${tab === t.key ? "bg-[#124734] text-white border-[#124734]" : "bg-white text-[#124734] border-[#A7E1B2]/60 hover:bg-[#ECF5EE]"}
                  `}
                >
                  {t.label}
                </button>
          ...
          <div className="mt-6 bg-white rounded-xl shadow">
            <div className="p-4 border-b flex items-center justify-between">
              ...
              <button onClick={load} className="text-sm font-semibold text-[#124734] underline">Refresh</button>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                      ...
                      <td className="p-3">
                        <div className="flex flex-wrap gap-1">
                          {(s.categories || []).slice(0, 3).map((c) => (
                            <span key={c} className="px-2 py-1 rounded bg-gray-100 text-xs">{c}</span>
                          ))}
                          {(s.categories || []).length > 3 ? (
                            <span className="px-2 py-1 rounded bg-gray-100 text-xs">+{(s.categories || []).length - 3}</span>
                          ) : null}
                        </div>
                      </td>
                      <td className="p-3">
                  ...
                  {loading && (
                    <tr>
                      <td colSpan={6} className="p-8 text-center text-gray-600">Loading suppliers…</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          {/* Details Drawer */}
          {selected && (
            <div className="fixed inset-0 bg-black/40 z-[2000] flex justify-end" onClick={() => setSelected(null)}>
              <div className="w-full sm:w-[520px] h-full bg-white p-6 overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-start justify-between gap-4">
                  ...
                  <button className="text-sm font-semibold text-[#124734] underline" onClick={() => setSelected(null)}>Close</button>
                </div>
                ...
                <div className="mt-6 grid grid-cols-1 gap-4">
                  ...
                  <div className="bg-gray-50 border rounded-xl p-4">
                    ...
                    <p className="text-sm text-gray-700">
                      {selected.pickupAddress?.city || "-"}, {selected.pickupAddress?.state || "-"} - {selected.pickupAddress?.pincode || "-"}
                    </p>
                  ...
                  <div className="bg-gray-50 border rounded-xl p-4">
                    ...
                    <p className="text-sm text-gray-700">GSTIN: <b>{selected.kyc?.gstin || "-"}</b></p>
                    <p className="text-sm text-gray-700">PAN: <b>{selected.kyc?.pan || "-"}</b></p>
                  </div>

                  <div className="bg-gray-50 border rounded-xl p-4">
                    <p className="text-sm font-bold text-gray-800 mb-2">Bank</p>
                    <p className="text-sm text-gray-700">Account Holder: <b>{selected.bank?.accountHolderName || "-"}</b></p>
                    <p className="text-sm text-gray-700">Account No: <b>{selected.bank?.accountNumber || "-"}</b></p>
                    <p className="text-sm text-gray-700">IFSC: <b>{selected.bank?.ifsc || "-"}</b></p>
                    <p className="text-sm text-gray-700">Bank: <b>{selected.bank?.bankName || "-"}</b></p>
                  </div>
