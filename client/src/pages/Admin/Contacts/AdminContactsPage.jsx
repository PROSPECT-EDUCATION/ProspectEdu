...
export default function AdminContactsPage() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await api.get("/contacts/admin", { ...authHeaders, params: { status, q } });
        ...
        <div className="px-6 pt-[90px] pb-10 overflow-y-auto text-left">
          <div className="bg-white rounded-2xl border shadow-sm p-5 mb-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-[#124734]">Contact Us Submissions</h2>
                <p className="text-sm text-gray-600">View all requests and update their status.</p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search name/email/phone/issue" className="border rounded-xl px-4 py-2 w-full sm:w-[280px]" />

                <button onClick={fetchItems} className="px-5 py-2 rounded-xl bg-[#124734] text-white font-semibold hover:bg-[#0B2F23]">
                  Search
                </button>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border shadow-sm overflow-hidden">
            <div className="p-5 border-b flex items-center justify-between">
              ...
              <button onClick={fetchItems} className="px-4 py-2 rounded-xl border hover:bg-gray-50 font-semibold text-sm">
                Refresh
              </button>
            </div>
            ...
            {loading ? (
              ...
              <div className="p-6 text-gray-600">No contact requests found.</div>
            ) : (
                  ...
                  <tbody className="divide-y">
                    {items.map((x) => (
                      <tr key={x._id} className="hover:bg-gray-50 align-top">
                        ...
                        <td className="px-4 py-3">
                          <div className="text-gray-800 whitespace-pre-line max-w-[520px]">{x.message}</div>
                        </td>
                        <td className="px-4 py-3">
                          <div className={badge(x.status)}>{x.status}</div>
                          <select className="mt-2 border rounded-lg px-2 py-1 text-xs" value={x.status} onChange={(e) => updateStatus(x._id, e.target.value)}>
                            <option value="PENDING">PENDING</option>
