...
export default function CheckoutPage() {
  ...
  const handlePayNow = async () => {
    ...
    try {
      ...
      // 2) for now: simulate payment success
      await purchasesApi.confirm(purchaseId, { providerPaymentId: "manual_success" });
  ...
  // Replace Pay Now button with:
  // onClick={handlePayNow} disabled={paying}

 if (loading) return <p className="text-center py-20">Loading checkout...</p>;
  if (error || !course) return <p className="text-center py-20 text-red-500">{error || "Course not found"}</p>;
