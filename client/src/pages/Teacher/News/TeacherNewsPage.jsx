...
const fmtToday = () => new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" });
...
export default function TeacherNewsPage() {
  ...
  const loadNews = async () => {
    setLoading(true);
    try {
      const res = await api.get("/news/teacher/list", { ...authHeaders, params: { category: filterCat, q } });
  ...
  useEffect(() => { loadCats(); }, []);
  useEffect(() => { loadNews(); /* eslint-disable-next-line */ }, [filterCat]);

  const resetForm = () => {
    setForm({ title: "", description: "", category: "", date: fmtToday(), english: "", hindi: "", img: null });
    setEditing(null);
  };
        ...
        <div className="px-6 pt-[90px] pb-10 overflow-y-auto text-left">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white border rounded-2xl shadow-sm p-5 flex items-center justify-between">
              ...
              <button onClick={openCreate} className="px-5 py-2 rounded-xl bg-[#124734] text-white font-semibold hover:bg-[#0B2F23]">
                + Add
              </button>
            </div>
          </div>
          ...
          <div className="bg-white border rounded-2xl shadow-sm p-5 mb-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              ...
              <div className="flex gap-2 w-full md:w-auto">
                ...
                />
                <button onClick={addCategory} className="px-5 py-2 rounded-xl bg-[#009846] text-white font-semibold hover:bg-green-700">
                  Add
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {cats.map((c) => (
                <div key={c._id} className="flex items-center gap-2 px-3 py-2 border rounded-xl bg-gray-50">
                  <span className="text-sm font-semibold text-[#124734]">{c.name}</span>
                  <button onClick={() => deleteCategory(c._id)} className="text-xs px-2 py-1 rounded-full bg-red-600 text-white">
                    Delete
                  </button>
                </div>
              ))}
          ...
          <div className="bg-white border rounded-2xl shadow-sm p-5 mb-4">
            <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <select value={filterCat} onChange={(e) => setFilterCat(e.target.value)} className="border rounded-xl px-4 py-2">
                  <option value="All">All</option>
                  {cats.map((c) => <option key={c._id} value={c.name}>{c.name}</option>)}
                </select>

                <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search title/desc/category" className="border rounded-xl px-4 py-2 w-full sm:w-[280px]" />

                <button onClick={loadNews} className="px-5 py-2 rounded-xl bg-[#124734] text-white font-semibold hover:bg-[#0B2F23]">
                  Search
                </button>
              </div>
            </div>
          ...
          <div className="bg-white border rounded-2xl shadow-sm overflow-hidden">
            <div className="p-5 border-b flex items-center justify-between">
              ...
              <button onClick={loadNews} className="px-4 py-2 rounded-xl border hover:bg-gray-50 font-semibold text-sm">
                Refresh
              </button>
            </div>
            ...
            {loading ? (
              ...
              <div className="divide-y">
                {news.map((n) => (
                  <div key={n._id} className="p-5 flex flex-col md:flex-row md:items-center gap-4 hover:bg-gray-50">
                    <img
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => openEdit(n)} className="px-4 py-2 rounded-xl border font-semibold hover:bg-white">
                        Edit
                      </button>
                      <button onClick={() => deleteNews(n._id)} className="px-4 py-2 rounded-xl bg-red-600 text-white font-semibold hover:bg-red-700">
                        Delete
                      </button>
                    </div>
                  </div>
            ...
            <div className="fixed inset-0 bg-black/40 z-[9999] flex items-center justify-center p-4">
              <div className="bg-white w-full max-w-3xl rounded-2xl shadow-xl border p-6">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-[#124734]">{editing ? "Edit News" : "Add News"}</h3>
                  <button onClick={() => { setOpen(false); resetForm(); }} className="px-3 py-1 rounded-lg border hover:bg-gray-50">
                    ✕
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  <div>
                    ...
                    <input className="w-full border rounded-xl px-4 py-2 mt-1" value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} />
                  </div>

                  <div>
                    <label className="text-sm font-semibold text-gray-700">Date (dd MMM yyyy)</label>
                    <input className="w-full border rounded-xl px-4 py-2 mt-1" value={form.date} onChange={(e) => setForm((p) => ({ ...p, date: e.target.value }))} />
                  </div>

                  <div className="md:col-span-2">
                    <label className="text-sm font-semibold text-gray-700">Short Description</label>
                    <textarea className="w-full border rounded-xl px-4 py-2 mt-1 min-h-[80px]" value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} />
                  </div>

                  <div>
                    <label className="text-sm font-semibold text-gray-700">Category</label>
                    <select className="w-full border rounded-xl px-4 py-2 mt-1" value={form.category} onChange={(e) => setForm((p) => ({ ...p, category: e.target.value }))}>
                  ...
                  <div>
                    ...
                    <input type="file" accept="image/*" className="w-full border rounded-xl px-4 py-2 mt-1" onChange={(e) => setForm((p) => ({ ...p, img: e.target.files?.[0] || null }))} />
                  </div>

                  <div className="md:col-span-2">
                    <label className="text-sm font-semibold text-gray-700">English Content</label>
                    <textarea className="w-full border rounded-xl px-4 py-2 mt-1 min-h-[120px]" value={form.english} onChange={(e) => setForm((p) => ({ ...p, english: e.target.value }))} />
                  </div>

                  <div className="md:col-span-2">
                    <label className="text-sm font-semibold text-gray-700">Hindi Content</label>
                    <textarea className="w-full border rounded-xl px-4 py-2 mt-1 min-h-[120px]" value={form.hindi} onChange={(e) => setForm((p) => ({ ...p, hindi: e.target.value }))} />
                  </div>
                </div>

                <div className="flex justify-end gap-2 mt-5">
                  <button onClick={() => { setOpen(false); resetForm(); }} className="px-5 py-2 rounded-xl border font-semibold hover:bg-gray-50">
                    Cancel
                  </button>
                  <button onClick={saveNews} className="px-6 py-2 rounded-xl bg-[#124734] text-white font-semibold hover:bg-[#0B2F23]">
