...
export default function SupplierSidebar({ isCollapsed, setIsCollapsed }) {
  ...
  return (
    <aside
      className={`bg-[#124734] text-white min-h-screen h-auto flex flex-col justify-between shadow-lg transition-all duration-300 
      ${isCollapsed ? "w-20" : "w-64"}`}
    >
      {/* LOGO */}
      <div>
        <div
          className={`flex items-center gap-3 px-6 py-6 border-b border-[#A7E1B2]/40 ${
        ...
        >
          ...
          {!isCollapsed && (
            <h2 className="text-xl font-semibold text-[#A7E1B2]">
              Supplier Panel
            </h2>
          )}
        </div>
      ...
      <div className="p-4 border-t border-[#A7E1B2]/30 flex justify-center">
        <button
          ...
          className="p-2 rounded-full bg-[#009846]/80 hover:bg-[#009846]"
        >
          {isCollapsed ? "▶" : "◀"}
        </button>
      </div>
