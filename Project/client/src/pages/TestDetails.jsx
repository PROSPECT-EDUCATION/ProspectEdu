...
const TestDetails = () => {
  ...
  return (
    <section className="bg-[#F9FAFB] text-[#124734] font-[Open_Sans,sans-serif]">
        ...
        <div className="max-w-7xl mx-auto flex  justify-between items-center px-8">
          ...
          <div className="w-full md:w-1/2 pr-8">
            <p className="text-sm mb-3 opacity-80">
              Home &gt; Test Series &gt; {test.title}
            </p>
            <h1 className="text-4xl font-semibold mb-4">{test.title}</h1>
          ...
          {/* Right Image */}
          <div className="w-full md:w-1/2 flex justify-end relative mt-8 md:mt-0">
            <img
              src={testImg}
              alt={test.title}
      ...
      {/* Schedule + Right Card Section */}
      <div className="max-w-7xl mx-auto mt-12 px-8 flex flex-col md:flex-row gap-8">
        {/* Left: Schedule */}
        <div className="bg-white rounded-xl shadow-md p-6 w-full md:w-2/3">
          ...
          <div className="divide-y divide-gray-200">
            {schedule.map((test) => (
              <div
                key={test.id}
                className="flex flex-col sm:flex-row sm:items-center justify-between py-4 px-2 hover:bg-[#F9FAFB] transition"
              >
                <div className="flex items-start sm:items-center gap-3">
                  <span className="text-purple-700 text-xl">🧾</span>
                  <div>
                    <p className="font-semibold text-[#124734]">{test.name}</p>
                    <p className="text-gray-600 text-sm flex gap-4 items-center mt-1">
                      <span>📅 {test.date}</span>
                      <span>⏰ {test.time}</span>
                    </p>
                  </div>
                ...
                <button
                  disabled
                  className={`px-4 py-1 rounded-full text-sm font-medium shadow-sm cursor-default ${getStatusColor(
                    test.status
                  )}`}
                >
                  {test.status}
        ...
        {/* Right: Test Info Card */}
        <div className="bg-white rounded-xl shadow-md p-6 w-full md:w-1/3 h-fit">
          <h3 className="text-3xl font-semibold mb-4">{test.title}</h3>
          <p className="text-gray-700 text-m mb-2">
            ...
            {test.id === "1"
              ...
              : "₹999/-"}{" "}
            {test.id !== "1" && (
              <span className="line-through text-gray-400 ml-1">3999/-</span>
            )}
          </p>
